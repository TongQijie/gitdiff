...
namespace GitDiff
{
    ...
    public class Git : IGit
    {
        ...
        public Git()
        {
            _Command = DependencyContainer.Resolve<IConfig>().GitPath;
        }
        ...
        public Change[] GetCommitDetail(string commitId)
        {
            ...
            using (var reader = process.ReadStream(DependencyContainer.Resolve<IConfig>().LocalGitDirectory))
            {
                string line = null;
        ...
        public string GetFileContent(string commitId, string path)
        {
            return new ProcessHelper(_Command).Add("show").Add($"{commitId}:\"{path}\"")
                .ReadString(DependencyContainer.Resolve<IConfig>().LocalGitDirectory);
        }
        ...
        {
            var config = DependencyContainer.Resolve<IConfig>();
